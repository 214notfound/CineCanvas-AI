...
CineCanvas 把调色拆成可练习的三层能力：

| 层 | 学什么 | 当前载体 |
|---|---|---|
| **机制** | 每个参数控制什么、彼此如何区别 | 四组辨析关卡 + 直方图示波器 |
| **操作** | 调色顺序、如何精确逼近目标观感 | AI 分步教案 + 工作台（匹配闭环开发中） |
| **品味** | 读懂并复现电影/大师风格 | 风格库 API 已就绪，UI 开发中 |

AI 只放在它可靠的位置——诊断、解说、风格适配；教学质量尽量由确定性内容（固定诊断图、人工校准配方、可验证的渲染数学）托底。
...
- **WebGL 实时调色引擎**（PixiJS v8 + 自定义 GLSL）：10 个 Lightroom 式基础滑块，线性光管线（sRGB↔linear → 白平衡增益 → 曝光 → 影调曲线 → 基础 S 曲线 → sigmoid 对比度 → 软肩 → vibrance/sat）
- **亮度曲线**：控制点 → 单调三次样条 → 256×1 LUT，shader 追加查表；默认 identity（追加式，不扰动既有 10 滑块数学）
- **确定性影调验收**：`npm run verify:tonal`——CPU 镜像 shader（`tonalMath.ts`），覆盖黑点锚定、高光软肩斜率、阴影提亮不脱色、中性 profile ≠ identity
- **AI 照片诊断 + 结构化教案**：Gemini / Kimi 可切换；zod schema 校验 → 带错误反馈的修复重试 → 纯文字降级，UI 不因坏 JSON 崩掉
- **工作台教学辅助**：分步 StepCoach、滑块教案目标区间高亮
- **教学实验室 `/learn/lab`**：before/after 闪回（关滤镜看真原图）、教学化直方图（渐变轴 / 幽灵基线 / 整句翻译）
- **第一层四辨析关**：曝光 vs 白色 vs 高光、阴影 vs 黑色、自然饱和度 vs 饱和度、色温 vs 色调（固定诊断图，无 AI）
- **会话持久化**：滑块 / 教案 / 步骤刷新保留（图片需重传）

### 开发中

- **电影风格库**：`generateFilmSteps` 已实现；量化风格卡（`films.ts`）+ 选卡入口待接
- **匹配目标图闭环**：并排 + 闪回比对，按图像差异评分（非滑块数值）
- **HSL 分通道**与曲线/HSL 专项关
- **首页与路由信息架构**：入口与板块关系还在收敛
- **清晰度家族**（清晰度 / 纹理 / 锐化）：需 RenderTexture 多 pass，刻意未塞进当前单 shader
