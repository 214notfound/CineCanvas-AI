...
**已从范围剔除（勿加回）：** 影响区域可视化、差异热力图、直方图↔照片像素联动、A/B 辨认题。

---

## P1.3 状态

- [x] 亮度曲线：控制点 → 单调三次样条 → 256×1 LUT；shader 追加 stage；默认 identity no-op
- [x] `tonalMath` + `verify:tonal` 覆盖 identity / 单调性
- [x] `/learn/lab` 曲线面板（S 曲线预设）
- [ ] RGB 分通道曲线（可后续紧随）
...
- 关卡数据：`src/data/lessons/`（id、标题、诊断图路径、对比文案、练习提示）
...
**验收：** 四关可从头练到标完成；不打开 `/workspace` 也能学完机制层这四组。
