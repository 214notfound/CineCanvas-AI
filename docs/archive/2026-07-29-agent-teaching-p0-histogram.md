...
**P0 已砍：** 影响区域可视化（与像素联动、差异热力图同属「默认不做」）。

**已完成：** 引擎响应幅度回归（原 P1.0）——不再列任务。

**P1 分步（见 `docs/plans/2026-07-29-p1-implementation-steps.md`）：**

- [ ] P1.1 持久化 / **P1.2 四辨析关** / P1.3–1.4 曲线+HSL / P1.5 专项关 / P1.6 匹配闭环
...
1. **刷新丢会话**：zustand 未 persist；换图/刷新 blob URL 会变（→ P1.1）。  
2. **AI 降级文案**曾引用不存在的「色彩分级/污点修复」——改 prompt 时只写产品内真实功能。  
3. **HMR + shader**：改 `grading.ts` 后硬刷新（旧坑仍在）。  
4. **中性 ≠ 原图**：基础 S 曲线刻意存在；before/after 已用关滤镜规避误解。  
5. **仓库里有 `.tools/cloudflared.exe`**（体积大）：注意勿误提交到不该去的远程策略；gitignore 视团队要求。  
6. **working tree**：若本地还有未提交文件，以 `git status` 为准（直方图相关已有 commit `b030720`）。

---

## 8. 下一步开发计划

**下一步：按 P1 分步文档做** [`docs/plans/2026-07-29-p1-implementation-steps.md`](../plans/2026-07-29-p1-implementation-steps.md)

顺序：P1.1 持久化 → **P1.2 四辨析关（主里程碑）** → 曲线/HSL → 匹配闭环。

**不要做：**  
- 影响区域可视化 / TonalRangeOverlay / 差异热力图主路径 UI（均已否决）
